...
function buildAutonomousAgentSection(loopMaxMinutes: number): string {
  return [
    ...
    "Before EVERY commit, ALL of the following MUST pass. Do NOT commit if any fail:",
    "1. Build compiles cleanly with zero errors",
    "2. ALL tests pass (no failures, no skipped tests)",
    "3. Linter reports zero warnings and zero errors",
    "If any check fails, fix the issue BEFORE committing. Never commit broken code.",
    "",
    ...
    "- There are linter warnings or errors",
    "- There are failing tests",
    "- The build does not compile cleanly",
...
function buildLoopSummarySection(): string {
  return [
    ...
    "",
    "CRITICAL: Coverage, cyclomatic complexity, AND linter cleanliness are ALL PRIMARY quality signals.",
    "Test count alone is NOT sufficient. You MUST measure and report ALL metrics every iteration.",
    "Linter warnings are as unacceptable as failing tests or build errors. Fix them immediately.",
